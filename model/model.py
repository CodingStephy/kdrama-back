...
class Actor(Base):
    ...
    def delete_actor(id):
        ...
        user = session.query(Actor).filter(id == id).first()
        session.delete(user)
        session.commit()
        session.close()
    ...
    def get_all_data():
        ...
        all_data = [{'id': row.id, 'name': row.name, 'photo': row.photo}
                    for row in sql_query_data]
...
class Show(Base):
    ...
    def delete_show(id):
        ...
        user = session.query(Show).filter(id == id).first()
        session.delete(user)
        session.commit()
        session.close()
...
def get_connection_string():
    return f'postgresql://bhvsxnqocgpohm:572acf071c282fc6861cae00f7b5885d9522fe94e6c7760c4962b6037bcbf281@ec2-54-158-247-97.compute-1.amazonaws.com:5432/d9garejd9cf2ih'
